...
function App() {
    ...
    fetch(`${window.API_BASE_URL}/progress/continue-watching?profile_id=${pid}`)
      ...
      .then(d => {
        const items = (d.data || []).map(it => {
          ...
          // Prefer the preloaded catalog item when available (carries a few
          // extra fields like a curated `genre`), but build the item
          // straight from this response otherwise instead of dropping it.
          // Continue-watching titles are frequently NOT in the small
          // preloaded home catalogs (hero/trending/top10/recommended/
          // movies/tv/anime/upcoming) — silently hiding anything missing
          // from that list was the actual bug (Library's own Continue
          // Watching never had this dependency, which is why it always
          // showed everything correctly).
  ...
  // Hero rotation and the #bg sync effect below both need to stop touching
  // the DOM once we've navigated off Home — they're declared unconditionally
  // (before any early return, since hooks can't be conditional) so without
  // this guard they keep running on Search/Detail/Library/etc. too. #bg is
  // display:none on those routes, but React reconciles the *same* #bg DOM
  // node across the Home <-> other-page branches (matching position in the
  // returned tree), so there's a real window where this effect's imperative
  // `bg.style.backgroundImage = ...` write lands on that shared node around
  // a route change and the hero image visibly flashes in behind the nav
  // bar on other pages before display:none reasserts itself.
  const isHomeRoute = !route.page || route.page === 'home';

  /* ── Auto-rotate hero (only while actually on Home) ──── */
  React.useEffect(() => {
    if (!isHomeRoute || heroPaused || heroList.length === 0) return;
    const speed = (t.heroSpeed || 8) * 1000;
    const id = setInterval(() => setHeroIdx(i => (i + 1) % heroList.length), speed);
    return () => clearInterval(id);
  }, [isHomeRoute, heroList, heroPaused, t.heroSpeed]);

  const hero = heroList[heroIdx] || heroList[0] || null;

  /* ── Sync #bg and --glow with hero (only while actually on Home) ──── */
  React.useEffect(() => {
    if (!isHomeRoute || !hero) return;
  ...
  if (route.page && route.page !== 'home' && routerPages[route.page]) {
    return (
      <React.Fragment>
        ...
        {/* BottomBar is position:fixed — must live outside .app, which sets
            overflow:hidden + isolation:isolate. Those don't spec-legally
            change position:fixed's containing block, but real-world mobile
            Safari has long-standing bugs where a fixed descendant of an
            overflow-clipped/isolated ancestor detaches from the viewport
            and appears to scroll with the page instead, landing wherever
            mid-scroll. BackToTopButton already lived outside .app for the
            same reason; BottomBar never got the same treatment. */}
        <BottomBar key="bottombar" active={mobileScreen} onNav={handleMobileNav}/>
        <BackToTopButton key="backtotop"/>
        <TweaksPanel key="tweaks">
  ...
  return (
    <React.Fragment>
      ...
      {/* Outside .app for the same reason as the router-pages branch —
          see the comment there. Keys match across both branches so React
          doesn't reuse a DOM node across a home <-> other-page switch. */}
      <BottomBar key="bottombar" active={mobileScreen} onNav={handleMobileNav}/>
      <BackToTopButton key="backtotop"/>
      <TweaksPanel key="tweaks">
        <TweakSection label="Style"/>
